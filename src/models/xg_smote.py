...
def train_model(X_train, y_train, args):
    ...
    preds = best_model.predict(X_train_res)
    probs = best_model.predict_proba(X_train_res)[:, 1]
    ...
    print("\nConfusion Matrix:")
    print(confusion_matrix(y_train_res, preds))

    print(f"\nROC AUC Score on Training Data: {roc_auc_score(y_train_res, probs):.4f}")
    return best_model

# Evaluate model on test data and print classification report and confusion matrix
